...
def ajax_profit_per_date_in_project(request, employee_id, project_id):
    ...
    index = 0
    for logDate in dates:
        logs = TimeLog.objects.filter(employee_id=employee_id, employee_id__projectdepartmentemployeerole__projectDepartment_id__project_id=project_id,
                                      workDate__year=logDate.year, workDate__month=logDate.month
                                      , workDate__day=logDate.day).distinct()
        expenses = logs.aggregate(
            total_expenses=Sum(F("duration") / 60.0 * F("employee_id__price_per_hour"), output_field=FloatField()))[
...
def ajax_get_employee_projects(request,employee_id):
    employee = get_object_or_404(Employee, id=employee_id)
    projects = Project.objects.filter(projectdepartment__projectdepartmentemployeerole__employee_id=employee_id,deleted=False)
