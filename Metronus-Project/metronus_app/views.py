...
def bad_request(request):
    """ Error 400 """
    return error_view(request, 'error400.html')

def permission_denied(request):
    """ Error 403 """
    return error_view(request, 'error403.html')

def page_not_found(request):
    """ Error 404 """
    return error_view(request, 'error404.html')

def error(request):
    """ Error 500 """
    return error_view(request, 'error500.html')

def error_view(request, html_file):
    """ Returns an error view """
    return render(request, "error/" + html_file, {'img' : error_gif()})
